...
describe('posts', () => {
  ...
  scenario('creates a post', async () => {
    const result = await createPost({
      input: { code: 'String309391', title: 'String', body: 'String' },
    })

    expect(result.code).toEqual('String309391')
  ...
  scenario('updates a post', async (scenario: StandardScenario) => {
    ...
    const result = await updatePost({
      id: original.id,
      input: { code: 'String42500682' },
    })

    expect(result.code).toEqual('String42500682')
  })
